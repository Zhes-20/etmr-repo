...
| `minApiVersion` | — | Минимальная версия EMRAPI (сейчас: 2). Приложения с `minApiVersion: 2` не запустятся на старых клиентах платформы. |
| `permissions` | — | Массив строк — запрашиваемые разрешения (см. ниже). |
| `photonAppId` | для `multiplayer` | Photon **Realtime** App ID игры (dashboard.photonengine.com → Create App → Realtime). |
| `photonVoiceAppId` | для голосового чата | Photon **Voice** App ID (второе приложение в дашборде → Create App → Voice). Голос без него не включится. |
| `photonRegion` | — | Регион Photon (`eu` по умолчанию, `us`, `asia`…). Должен быть ОДИНАКОВЫМ у всех игроков — и для Realtime, и для Voice. |
...
| `multiplayer` | `EMRAPI.Multiplayer` — онлайн-комнаты, сетевые объекты, голосовой чат (`SetVoiceEnabled` и микрофон) |
...
### 5.1 Формат пакета `.emr`

`.emr` — зашифрованный контейнер. Раскладка актуального формата **EMRC**:

| Смещение | Размер | Содержимое |
|---|---|---|
| 0 | 4 | Magic `EMRC` |
| 4 | 16 | IV (AES-CTR) |
| 20 | 32 | HMAC-SHA256 поверх `magic + IV + ciphertext` |
| 52 | … | Зашифрованный ZIP (AES-128-CTR) |

- Ключи выводятся на клиенте из материала, разнесённого по рантайму (PBKDF2-SHA256, 10000 раундов); в боте-сборщике и клиенте они должны совпадать. **Никогда не документировать и не коммитить исходный ключевой материал** — в документации описана только раскладка.
- Любая подмена заголовка или байтов пакета ломает HMAC — пакет отклоняется до расшифровки.
- Старый формат **EMRB** читается клиентом только для уже установленных пакетов; новые сборки всегда EMRC.
- `.emrd` — незашифрованный ZIP, только для быстрой отладки (см. MAINSYSTEMPROMPT). Не для распространения.
- Версионность платформы: `API_VERSION` клиента и `minApiVersion` приложения сверяются при установке/запуске. Приложение с `minApiVersion` выше, чем у клиента, отклоняется («App requires API vN»).
...
**Быстрый тест на устройстве без бота (`.emrd`):**
запакуй содержимое папки в `.zip`, переименуй в `.emrd` и открой файл на телефоне (Файлы / «Поделиться» → EnJoyTheMR) — платформа покажет диалог установки. `.emrd` — тот же пакет, но без шифрования; формат только для разработки, в магазин идёт `.emr` от бота.
