...
## Онлайн-игра — EMRAPI.Multiplayer (Чистый Lua, Scene Mode)

Комната на четверых, кидаемся шарами. Всё в одном файле: подключение, комната,
сетевые шары с физикой, события. Аватары игроков появляются сами.

**`app.json`**
```json
{
  "id": "com.example.ballroom",
  "name": "Ball Room",
  "version": "1.0.0",
  "entry": "main.lua",
  "display": "scene",
  "type": "lua",
  "photonAppId": "ВАШ-APPID-ИЗ-PHOTON-DASHBOARD",
  "photonVoiceAppId": "ВАШ-VOICE-APPID (второе приложение типа Voice)",
  "permissions": ["multiplayer", "scene", "input"]
}
```
...
local lastPinch = false

function OnStart()
    MP:Connect(function(ok, err)
        if not ok then EMRAPI:LogError("connect: " .. tostring(err)) return end
        MP:JoinRandomOrCreateRoom(4, function(ok2, err2)
            if ok2 then EMRAPI:Log("в комнате: " .. MP:GetRoomName()) end
            -- Голосовой чат: включаем после входа в комнату.
            -- Голос идёт через отдельный Photon AppId типа Voice (photonVoiceAppId).
            local v = MP:SetVoiceEnabled(true)
            if v ~= "голос включён" and v ~= "голос уже включён" then
                EMRAPI:LogWarning("голос: " .. v)
            end
        end)
    end)

    MP:OnPlayerJoined(function(actor, nick) EMRAPI:Log(nick .. " зашёл") end)
    MP:OnEvent("hello", function(sender, json)
        EMRAPI:Log("привет от #" .. sender)
    end)

    -- Индикация «кто говорит»: подсвечивай аватар/ник.
    MP:OnPlayerSpeaking(function(actor, isSpeaking)
        if isSpeaking then EMRAPI:Log("#" .. actor .. " говорит") end
    end)
end

function OnUpdate(dt)
    if not MP:IsInRoom() then return end
    local pinch = EMRAPI.Input:GetHandGesture("right") == "Pinch"
    if pinch and not lastPinch then
        local x, y, z = EMRAPI.Input:GetHandPosition("right", "PalmCenter")
        local ball = MP:SpawnSynced("sphere", x, y, z, 0.12)  -- видят все, и поздние тоже
        local rb = EMRAPI.Scene:AddRigidbody(ball, 0.3, true) -- физика у владельца
        local cam = CS.UnityEngine.Camera.main
        if rb and cam then rb.velocity = cam.transform.forward * 4.0 end
        MP:SendEvent("hello", "{}")
    end
    lastPinch = pinch
end
...
function OnDestroy()
    -- Дисконнект и чистка сетевых объектов — автоматически.
end
```

**Ключевые паттерны:**
- `photonAppId` разработчик получает сам: dashboard.photonengine.com → Create App → Realtime.
- `SpawnSynced` + `AddRigidbody`: физика у владельца, у остальных плавная интерполяция.
- Поймал чужой объект — `MP:TakeOwnership(obj)`, теперь физика твоя (так делается ловля мяча).
- Игровая логика — через `SendEvent`/`OnEvent` с JSON, состояние — через Room/PlayerProperty.
- Аватары (голова+руки+ник) синкаются оболочкой сами; `MP:SetAvatarsEnabled(false)` —
  выключить и собрать свои (Timer:Repeat + Input:GetHeadPosition/GetHandPosition +
  SendEvent/OnEvent + любой рендер, хоть GLB-модель) — полный сниппет в разделе
  «Аватары игроков — два пути» справочника EMRAPI.Multiplayer.
