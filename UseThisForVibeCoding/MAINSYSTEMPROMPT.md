...
minApiVersion	Minimum platform API version (current: 2)
...
EMRAPI.Keyboard (no permission needed):
The platform's virtual keyboard. NOTE: for normal text input you do NOT need this API at all — tapping an EMRAPI.UI:CreateInputField focuses it, the keyboard appears automatically and types into the field.
Visibility: Show() force-show, Hide() force-hide, SetAuto() back to automatic (focus-driven), IsVisible() → bool
Transform: SetPosition(x,y,z) world coords, SetRotation(x,y,z), SetScale(factor) — multiplier of the ORIGINAL size (1 = default), GetPositionX/Y/Z()
Language: SetLanguage("en" | "ru"), GetLanguage()
Raw input capture (custom text UI without an input field): OnInput(function(text) end), OnBackspace(function() end), OnEnter(function() end), ClearCallbacks(). Callbacks fire ONLY when no input field is focused (focused field always wins). Call Show() yourself when capturing raw input.
Reset() restores everything; all changes auto-revert when the app closes.
...
Setting minApiVersion higher than current platform (currently 2)
